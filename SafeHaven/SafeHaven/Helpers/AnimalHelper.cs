...
namespace SafeHaven.Helpers
{
	public static class AnimalHelper
	{
		...
		private static string GetConnectionString()
		{
			...
			connectionString = ConfigurationManager.ConnectionStrings["AnimalDB"].ConnectionString;
		...
		internal static int? Edit(Animal animal)
		{
			...
			using (SqlConnection connection = new SqlConnection(GetConnectionString()))
			{
				...
				{
					string insert = "Insert into Animal (";
					...
					insert += ") ";
					insertValues += ") ";
					using (SqlCommand command = new SqlCommand(insert + insertValues, connection))
					{
						foreach (SqlParameter parameter in parameters)
						{
							command.Parameters.Add(parameter);
